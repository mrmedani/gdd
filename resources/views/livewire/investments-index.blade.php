<div>
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">
                {{ __('investments.title') }}
            </h1>
            <p class="text-slate-500 dark:text-slate-500 text-sm mt-1.5 font-medium">{{ __('investments.subtitle') }}</p>
        </div>
        @if(!$showForm)
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        @foreach($typeCards as $card)
            @php
                $colors = [
                    'equipment' => ['bg' => 'bg-amber-50 dark:bg-amber-500/10', 'text' => 'text-amber-600 dark:text-amber-400', 'ring' => 'bg-amber-100 dark:bg-amber-500/20'],
                    'project' => ['bg' => 'bg-violet-50 dark:bg-violet-500/10', 'text' => 'text-violet-600 dark:text-violet-400', 'ring' => 'bg-violet-100 dark:bg-violet-500/20'],
                    'other' => ['bg' => 'bg-slate-50 dark:bg-slate-500/10', 'text' => 'text-slate-600 dark:text-slate-400', 'ring' => 'bg-slate-100 dark:bg-slate-500/20'],
                ];
            ...
            <button type="button" wire:click="filterByType('{{ $card['key'] }}')"
                ...
                <div class="flex items-center justify-between mb-3">
                    <div class="p-2.5 rounded-xl {{ $c['ring'] }}">
                        <svg class="w-5 h-5 {{ $c['text'] }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 17h8m0 0V9m0 8l-8-8-4 4-6-6"/></svg>
                    </div>
                    <span class="text-xs font-semibold {{ $active ? 'text-white dark:text-slate-200' : 'text-slate-400 dark:text-slate-500' }}">{{ $card['count'] }} {{ __('investments.entries') }}</span>
                </div>
    ...
    @if($showForm)
        <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-sm border border-slate-200/50 dark:border-slate-800/60 p-6 md:p-10 mb-8">
            <h2 class="text-lg font-bold text-slate-800 dark:text-white mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
                {{ $investmentId ? __('investments.edit') : __('investments.add') }}
            </h2>
            ...
            <form wire:submit="save" class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Date -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('investments.date') }} <span class="text-red-500">*</span></label>
                    ...
                    <!-- Amount -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('investments.amount') }} <span class="text-red-500">*</span></label>
                        <div class="relative">
                    ...
                    <!-- Type -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('investments.type') }} <span class="text-red-500">*</span></label>
                        <select wire:model.live="type" class="w-full px-4 py-3 bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-slate-700 dark:text-slate-300">
                            @foreach(self::TYPES as $key => $labelKey)
                                <option value="{{ $key }}">{{ __($labelKey) }}</option>
                            @endforeach
                        </select>
                    ...
                    <!-- Beneficiary -->
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('investments.beneficiary') }}</label>
                        <input type="text" wire:model="beneficiary" class="w-full px-4 py-3 bg-slate-50/50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none transition-all text-slate-700 dark:text-slate-300" placeholder="{{ __('investments.beneficiary_placeholder') }}">
                    </div>
                </div>

                <!-- Notes -->
                <div>
                    <label class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">{{ __('investments.notes') }}</label>
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-2.5 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 shadow-sm transition-colors cursor-pointer">
                        {{ $investmentId ? __('common.update') : __('common.save') }}
                    </button>
                </div>
            </form>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-slate-50/50 dark:bg-slate-950/30 text-slate-500 dark:text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-100 dark:border-slate-800/60">
                    <tr>
                        <th class="py-4 px-6 text-start">{{ __('investments.date') }}</th>
                        <th class="py-4 px-6 text-start">{{ __('investments.type') }}</th>
                        <th class="py-4 px-6 text-start">{{ __('investments.beneficiary') }}</th>
                        <th class="py-4 px-6 text-end">{{ __('investments.amount') }}</th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                    @forelse($investments as $investment)
                        <tr class="hover:bg-blue-50/20 dark:hover:bg-slate-800/30 transition-colors">
                            <td class="py-4 px-6 font-medium text-slate-700 dark:text-slate-300 whitespace-nowrap">{{ $investment->date->format('d/m/Y') }}</td>
                            <td class="py-4 px-6">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300">
                                    {{ \App\Domains\Treasury\Models\Investment::typeLabel($investment->type) }}
                                </span>
                            </td>
                            <td class="py-4 px-6 text-slate-600 dark:text-slate-400">{{ $investment->beneficiary ?? '-' }}</td>
                            <td class="py-4 px-6 font-bold text-rose-600 dark:text-rose-400 whitespace-nowrap text-end"><span dir="ltr">− {{ number_format($investment->amount, 2, ',', ' ') }} {{ getCurrency() }}</span></td>
                            <td class="py-4 px-6 text-end whitespace-nowrap">
                                <button wire:click="edit({{ $investment->id }})" class="text-blue-600 dark:text-blue-400 font-semibold hover:underline cursor-pointer">{{ __('common.edit') }}</button>
                                <button wire:click="delete({{ $investment->id }})" wire:confirm="{{ __('investments.confirm_delete') }}" class="text-rose-500 dark:text-rose-400 font-semibold hover:underline ms-3 cursor-pointer">{{ __('common.delete') }}</button>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="py-10 text-center text-slate-400 dark:text-slate-500">{{ __('investments.no_data') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
