<div class="max-w-6xl mx-auto space-y-8 animate-fade-in">
    ...
    <div class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-premium dark:shadow-premium-dark border border-slate-200/50 dark:border-slate-800/60 overflow-hidden relative z-10 hover:shadow-premium-hover dark:hover:shadow-premium-dark-hover transition-all duration-300">
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-slate-50/50 dark:bg-slate-950/30 text-slate-400 dark:text-slate-500 font-bold text-xs uppercase tracking-wider border-b border-slate-100 dark:border-slate-800/60">
                    <tr>
                        <th class="py-4 px-4 w-12">
                            <input type="checkbox" wire:click="toggleSelectAll" class="w-4 h-4 rounded accent-blue-600 cursor-pointer" />
                        </th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800/50 bg-transparent">
                    @forelse($backups as $backup)
                        <tr class="hover:bg-blue-50/20 dark:hover:bg-slate-800/30 transition-colors {{ in_array($backup['name'], $selectedBackups) ? 'bg-blue-50/40 dark:bg-blue-950/20' : '' }}">
                            <td class="py-4 px-4">
                                <input type="checkbox" wire:click="toggleSelect('{{ $backup['name'] }}')" {{ in_array($backup['name'], $selectedBackups) ? 'checked' : '' }} class="w-4 h-4 rounded accent-blue-600 cursor-pointer" />
                            </td>
                            <td class="py-4 px-6">
        ...
        @if(count($selectedBackups) > 0)
            <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-800/60 bg-blue-50/30 dark:bg-blue-950/20 flex flex-wrap items-center justify-between gap-3">
                <p class="text-sm font-bold text-slate-700 dark:text-slate-300">
                    {{ __('settings.backup_selected', ['count' => count($selectedBackups)]) }}
                </p>
                <div class="flex items-center gap-2">
                    <button wire:click="$set('selectedBackups', [])" class="px-3 py-2 text-xs font-bold rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer">
                        {{ __('common.cancel') }}
                    </button>
                    <button wire:click="confirmBulkDelete" class="inline-flex items-center gap-1.5 px-4 py-2 bg-red-600 text-white rounded-xl font-bold shadow-lg shadow-red-500/20 hover:shadow-red-500/40 hover:-translate-y-0.5 transition-all text-xs cursor-pointer">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                        {{ __('settings.backup_delete_selected') }}
                    </button>
                </div>
            </div>
        @endif
    ...
    <div x-data="{ show: @entangle('showBulkDeleteModal') }" x-show="show" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div x-show="show" x-transition.opacity class="absolute inset-0 bg-slate-950/60 backdrop-blur-sm" @click="show = false"></div>
        <div x-show="show" x-transition class="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200/50 dark:border-slate-800/60 w-full max-w-md p-6">
            <div class="flex items-center gap-3 mb-4">
                <div class="w-11 h-11 rounded-2xl bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01M5.07 19H19a2 2 0 001.74-3L14.74 4a2 2 0 00-3.48 0L4.26 16a2 2 0 001.74 3z" /></svg>
                </div>
                <div>
                    <h3 class="text-base font-bold text-slate-800 dark:text-white">{{ __('settings.backup_bulk_delete_title') }}</h3>
                    <p class="text-sm text-slate-500 dark:text-slate-400 font-medium">{{ __('settings.backup_bulk_delete_desc', ['count' => count($selectedBackups)]) }}</p>
                </div>
            </div>
            <div class="flex justify-end gap-2 mt-6">
                <button @click="show = false" class="px-4 py-2.5 text-sm font-bold rounded-xl border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all cursor-pointer">{{ __('common.cancel') }}</button>
                <button wire:click="deleteSelected" class="px-4 py-2.5 bg-red-600 text-white rounded-xl font-bold shadow-lg shadow-red-500/20 hover:shadow-red-500/40 transition-all text-sm cursor-pointer">{{ __('common.delete') }}</button>
            </div>
        </div>
    </div>
</div>
