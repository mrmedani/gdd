...
<head>
    <meta charset="utf-8">
    <title>{{ __('reports.annual_report') }} {{ $year }} - {{ $company['name'] }}</title>
    <style>
        ...
        .bar-bg { background: #f1f5f9; border-radius: 3px; height: 14px; overflow: hidden; width: 100%; }
        .bar-fill {
            height: 100%; border-radius: 3px; min-width: 2px;
            font-size: 7px; color: #fff; text-align: right; padding-right: 4px; line-height: 14px;
        }
...
<body class="{{ app()->getLocale() === 'ar' ? 'rtl' : '' }}">
        ...
        <table><tr>
            ...
            <td>
                <div class="kpi-card bg-blue">
                    ...
                    @if($prevTotal > 0)
                        ...
                        <div class="sub">
                            {{ $year - 1 }}: {{ formatMoney($prevTotal) }} {{ $company['currency'] }}
            ...
            <td>
                <div class="kpi-card bg-amber">
                    ...
                    <div class="sub">{{ $balance >= 0 ? __('reports.surplus') : __('reports.deficit') }}</div>
                </div>
            </td>
            <td>
                <div class="kpi-card bg-purple">
                    <div class="label">{{ __('reports.monthly_avg') }}</div>
                    <div class="value">{{ formatMoney($monthlyAvg) }} {{ $company['currency'] }}</div>
                    <div class="sub">{{ $expenses->count() }} {{ __('reports.operations') }}</div>
                </div>
            </td>
        </tr></table>
    </div>
    ...
    <div class="highlight-box">
        <strong>{{ $expenses->count() }}</strong> {{ __('reports.operations') }}
        &nbsp;&nbsp;|&nbsp;&nbsp;
        <strong>{{ $byMonth->filter(fn($m) => $m['count'] > 0)->count() }}</strong>/12 {{ __('reports.active_months') }}
        &nbsp;&nbsp;|&nbsp;&nbsp;
        @if($prevTotal > 0)
            @php $yearChange = $total - $prevTotal; @endphp
            <strong>{{ __('reports.vs_year') }} {{ $year - 1 }}:</strong>
            <span class="{{ $yearChange <= 0 ? 'variance-pos' : 'variance-neg' }}">
                {{ $yearChange >= 0 ? '+' : '' }}{{ formatMoney($yearChange) }} {{ $company['currency'] }}
                ({{ $prevTotal > 0 ? round(($yearChange / $prevTotal) * 100, 1) : 0 }}%)
            </span>
        @endif
    </div>
    ...
    @if(count($byMonth) > 0)
        <div class="section">
            ...
            <table class="data">
                <thead>
                    <tr>
                        <th style="width:60px">{{ __('reports.month') }}</th>
                ...
                <tbody>
                    ...
                    @foreach($byMonth as $m)
                        @php
                            ...
                            $barColor = $pct > 75 ? '#1e40af' : ($pct > 50 ? '#1d4ed8' : ($pct > 25 ? '#2563eb' : '#3b82f6'));
                            $annualPct = $total > 0 ? round(($m['total'] / $total) * 100, 1) : 0;
                        @endphp
                        <tr>
                            <td>{{ $m['label'] }}</td>
                            <td class="bar-cell">
                                <div class="bar-bg">
                                    <div class="bar-fill" style="width: {{ max($pct, 1) }}%; background: {{ $barColor }};">
                                        @if($pct > 30 && $m['total'] > 0){{ formatMoney($m['total']) }}@endif
                                    </div>
                                </div>
                            </td>
                            <td class="right" style="font-weight:bold;">{{ formatMoney($m['total']) }}</td>
                ...
                <tbody>
                    ...
                    @foreach($byCategory as $catName => $catData)
                        @php
                            ...
                            $barPct = $maxCatTotal > 0 ? ($catData['total'] / $maxCatTotal) * 100 : 0;
                            $catIdx++;
                        @endphp
                        <tr>
                            <td><span class="dot" style="background:{{ $color }}"></span> {{ $catName }}</td>
                            <td class="bar-cell">
                                <div class="bar-bg" style="height:10px;">
                                    <div style="width: {{ max($barPct, 1) }}%; background: {{ $color }}; height:100%; border-radius:3px; min-width:2px;"></div>
                                </div>
                            </td>
                            <td class="right" style="font-weight:bold;">{{ formatMoney($catData['total']) }}</td>
                            <td class="right">{{ $catData['percentage'] }}%</td>
                            <td class="center">{{ $catData['count'] }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot>
    ...
    @if($byEmployee->count() > 1)
        <div class="section">
            ...
            <table class="data">
                ...
                <tbody>
                    @php $maxEmpTotal = $byEmployee->first()['total']; @endphp
                    @foreach($byEmployee as $emp)
                        ...
                        <tr>
                            ...
                            <td class="bar-cell">
                                @if($barPct > 0)
                                    <div class="bar-bg" style="height:10px;">
                                        <div style="width: {{ max($barPct, 1) }}%; background: #14b8a6; height:100%; border-radius:3px; min-width:2px;"></div>
                                    </div>
                                @endif
                            </td>
                    ...
                    @foreach($byPaymentMethod as $method => $pmData)
                        ...
                        <tr>
                            ...
                            <td class="bar-cell">
                                @if($pmBarPct > 0)
                                    <div class="bar-bg" style="height:10px;">
                                        <div style="width: {{ max($pmBarPct, 1) }}%; background: {{ $pmColor }}; height:100%; border-radius:3px; min-width:2px;"></div>
                                    </div>
                                @endif
                            </td>
